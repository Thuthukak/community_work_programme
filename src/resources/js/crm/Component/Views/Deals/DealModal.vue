...
<!--            </div>-->
            ...
            <div class="col-sm-9" :key="renderContextableInput">
              <app-input
                v-model="formData.contextable_id"
...
<!--                  />-->
                  <app-input
                    ...
                    :placeholder="$t('choose_an_organization')"
                    type="search-and-select"
                    @input="handleOrgPersonChange"
                    :error-message="$errorMessage(errors, 'contextable_id')"
                  />
                </div>
              </div>
            </template>
            <div class="form-group row" v-if="formData.contextable_id">
              <label class="mb-0 col-sm-3 d-flex align-items-center">
                {{ $t("contact_person")  }}
              </label>
              <div class="col-sm-9">
...
<!--                />-->
                    ...
                    <template v-if="formData.contextable_id">
                        <app-input
                            :key="formData.contextable_id"
                            v-model="formData.person_id"
                            :options="personListAsOrgOptios"
                            :placeholder="$t('choose_a_contact_person')"
                            type="search-and-select"
...
<!--                  old-->
<!--                <small-->
<!--                  class="text-danger col-sm-9 pl-0 mb-0"-->
<!--                  v-if="formData.contextable_id && personListAsOrg.length < 1"-->
<!--                >-->
                <small
                  class="text-danger col-sm-9 pl-0 mb-0"
                  v-if="formData.contextable_id && contactPersonListLength < 1"
                >
        ...
        <div class="form-group row" v-if="$can('manage_public_access')">
          <label class="mb-0 col-sm-3 d-flex align-items-center">{{ $t("owner") }}</label>
          <div class="col-sm-9">
            <app-input
        ...
        <template v-if="customFields.length && customFieldDataLoaded">
          <div v-for="(field, index) in customFields" :key="index" class="form-group row">
            <div class="col-md-3">
            ...
            <div class="col-md-9">
              ...
              <template v-if="field.custom_field_type.name === 'number'">
                <app-input v-model="customFieldValue[field.name]" type="number" />
              </template>
...
export default {
  ...
  data() {
    ...
    organizationOptions: {
        ...
        // personListAsOrgOptios: {
        //     // url: route('selectable.organization_owners',{id: this.formData.contextable_id}),
        //     url: route('selectable.organization_owners',{id: 118}),
        //     query_name: "name", // by default 'search'
        //     per_page: 10, // must need to set min 10 per page for showing scrollbar
        //     modifire: (item) => {
        //         return { id: item.id, value: item.name }
  ...
  methods: {
      handleListChange(newLength) {
          this.contactPersonListLength = newLength;
      },
      handleOrgPersonChange(){
    ...
    submit() {
      ...
      this.addEditData.tax_id = this.formData.tax_id;

      this.addEditData.expired_at = this.formData.expired_at
        ? moment(this.formData.expired_at).format("YYYY-MM-DD HH:mm:ss")
        : null;
    ...
    getPipeline() {
      this.axiosGet(route("pipelines.index"))
        .then((response) => {
          this.pipelineList = this.collection(response.data.data).shaper("name");
    ...
    changeLeadType() {
      ...
      this.renderContextableInput = false;
      setTimeout(() => {
          this.renderContextableInput = true;
      }, 100)
    },
  },
};
