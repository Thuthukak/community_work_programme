@php
//Because I want to identify the ERROR of KR on which O, but I don’t know how to write it $errors
    $isError =false;
    if(count($errors)>0){
        ...
        <form method="POST" action="{{ route('kr.store') }}">
            ...
            <div class="form-row">
                ...
                <div class="form-group col-md-12">
                    <label for="keyresult_title">KeyResult <strong class="text-danger">{{ $isError ? $errors->first('krs_title') : '' }}</strong>  </label>
                    <input type="text" class="form-control" name="krs_title" id="keyresult_title" value="{{ old('krs_title') }}">
                </div>
                <div class="form-group col-md-6">
                    <label for="keyresult_confidence">Reaching rate <strong class="text-danger">{{ $isError ? $errors->first('krs_now'): '' }}</strong></label>
                ...
                <div class="form-group col-md-3">
                    <label for="keyresult_weight">Weights  <strong class="text-danger">{{ $isError ? $errors->first('krs_weight'): '' }}</strong> </label>
                ...
                <div class="form-group col-md-3">
                    <label for="keyresult_confidence">Confidence <strong class="text-danger">{{ $isError ? $errors->first('krs_conf'): '' }}</strong></label>
                ...
                <div class="form-group col-md-2">
                    <label for="keyresult_initaial">Start value  <strong class="text-danger">{{ $isError ? $errors->first('krs_init'): '' }}</strong></label>
                ...
                <div class="form-group col-md-6 u-text-right">
                    <button class="btn btn-primary u-mt-16" type="submit" style="width:100px;">Modify</button>   
                </div>
            </div>
        </form>
    </div>
