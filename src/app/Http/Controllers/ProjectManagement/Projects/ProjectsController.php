...
class ProjectsController extends Controller
{
    ...
    public function getProjectByFilter(Request $request, ProjectsFilter $filter)
    {
        ...
        if ($request->has('Organization')) {
            ...
            $filter->organization($request->get('Organization'));  // Apply the filter

        }elseif($request->has('minValue') || $request->has('maxValue')) {

            $filter->projectValue($request->get('minValue'), $request->get('maxValue'));  // Apply the filter

        }elseif($request->has('classes')) {

            $filter->classes($request->get('clauses'));  // Apply the filter

        }elseif ($request->has('startDate') || $request->has('endDate')) {
            $filter->createdAt($request->get('startDate'), $request->get('endDate'));  // Apply the filter
        }
