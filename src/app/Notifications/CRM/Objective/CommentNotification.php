...
class CommentNotification extends Notification implements ShouldQueue
{
    ...
    public function __construct($comment, $icon)
    {
        ...
        if ($comment->parent == null) {
            ...
            $this->details['name'] = is_a($users, 'App\Models\Core\Auth\User') ? $users->name : 'All';
            $this->details['body'] = ' Leave a message on' . $comment->commentable->getHasCommentMessage();
        } else {
            $users = $comment->parent->commenter;
            $this->details['name'] = $users->name;
            $this->details['body'] = ' Reply to your message';
